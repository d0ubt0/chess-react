...
import { getMoves } from "../chessLogic";
  ...
  for (let row = 0; row < 8; row++) {
    ...
    for (let col = 0; col < 8; col++) {
      if (row === 0) {
        ...
        if (col === 1 || col === 6)
          actualRow.push(new Piece("black", "knight"));
        if (col === 2 || col === 5)
          actualRow.push(new Piece("black", "bishop"));
        ...
        if (col === 1 || col === 6)
          actualRow.push(new Piece("white", "knight"));
        if (col === 2 || col === 5)
          actualRow.push(new Piece("white", "bishop"));
...
export default function ChessGame() {
  ...
  const clickFunction = (position: PositionType) => {
    ...
    if (
      (selected?.[0] === position[0] && selected?.[1] === position[1]) ||
    ...
    <section className="ChessGameContainer">
      {board.map((row, rowIndex) =>
        row.map((piece, colIndex) => {
          const isSelectedActual =
            selected?.[0] === rowIndex && selected?.[1] === colIndex;
          const isValidMoveActual = validMoves.some(
          ...
          return (
            <Square
              ...
              isSelected={isSelectedActual}
              isValidMove={isValidMoveActual}
            >
              {piece}
            </Square>
          );
